...
const CharacterEditor: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end">
      <div className="absolute inset-0 bg-black/20 backdrop-blur-sm" onClick={() => setCharacterEditorOpen(false)} />

      <div className="relative w-full max-w-5xl bg-ink-950 border-l border-ink-700 h-full flex flex-col animate-slide-in shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-ink-700 bg-ink-900 flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-lg font-display font-bold text-steel-100">
                {activeCharacterId ? 'Edit Dossier' : 'New Dossier'}
              </h2>
              <p className="text-[9px] text-steel-600 font-mono uppercase tracking-[0.15em]">
                {activeCharacterId ? `ID: ${activeCharacterId.split('-')[0]}` : 'CREATING'}
              </p>
            </div>
          </div>
          <button onClick={() => setCharacterEditorOpen(false)} className="p-2 text-steel-500 hover:text-white hover:bg-ink-700 rounded-full transition-all">
            <X size={22} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-hidden flex">
          {/* Left: Identity & Lore */}
          <div className="w-[320px] border-r border-ink-700 overflow-y-auto p-5 space-y-6 custom-scrollbar bg-ink-900/30 flex-shrink-0">
            {/* Core identity */}
            <section className="space-y-3">
              <div>
                <label className="block text-[9px] font-bold text-steel-600 uppercase tracking-[0.2em] mb-1.5">Full Name</label>
                <input
                  type="text" value={formData.name || ''}
                  onChange={e => setFormData({ ...formData, name: e.target.value })}
                  className="w-full bg-ink-950 border border-ink-700 rounded-lg px-3.5 py-2 text-steel-100 text-sm focus:border-char-400 focus:outline-none transition-all"
                  placeholder="Character Name"
                />
              </div>
              <div>
                <label className="block text-[9px] font-bold text-steel-600 uppercase tracking-[0.2em] mb-1.5">Archetype</label>
                <input
                  type="text" value={formData.archetype || ''}
                  onChange={e => setFormData({ ...formData, archetype: e.target.value })}
                  className="w-full bg-ink-950 border border-ink-700 rounded-lg px-3.5 py-2 text-steel-100 text-sm focus:border-char-400 focus:outline-none"
                  placeholder="e.g. The Reluctant Hero"
                />
              </div>
              <div>
                <label className="block text-[9px] font-bold text-steel-600 uppercase tracking-[0.2em] mb-1.5">Role</label>
                <select
                  value={formData.role || 'Supporting'}
                  onChange={e => setFormData({ ...formData, role: e.target.value as CharacterRole })}
                  className="w-full bg-ink-950 border border-ink-700 rounded-lg px-3.5 py-2 text-steel-100 text-sm cursor-pointer"
                >
            ...
            {/* Smart Tags */}
            <section className="space-y-3">
              <label className="flex items-center gap-1.5 text-[9px] font-bold text-steel-600 uppercase tracking-[0.2em]">
                <Hash size={10} /> Smart Tags
              </label>
              <div className="space-y-2">
                {Object.entries(formData.smart_tags || {}).map(([key, val]) => (
                  <div key={key} className="flex items-center gap-2">
                    <span className="text-[9px] text-steel-500 font-bold w-16 truncate flex-shrink-0">{key}</span>
                    <input
                      type="text" value={val}
                      onChange={e => setFormData({ ...formData, smart_tags: { ...formData.smart_tags, [key]: e.target.value } })}
                      className="flex-1 bg-ink-950 border border-ink-700 rounded px-2.5 py-1 text-xs text-steel-200"
                    />
                    <button onClick={() => removeSmartTag(key)} className="text-steel-600 hover:text-red-400 p-0.5">
                      <Trash2 size={10} />
                    </button>
                  </div>
                ))}
            ...
            <section className="space-y-2">
              ...
              <textarea
                ...
                className="w-full bg-ink-950 border border-ink-700 rounded-lg p-3 text-xs text-steel-200 resize-none"
              />
            </section>
            ...
            <section className="space-y-2">
              ...
              <div className="bg-ink-950 border border-ink-700 rounded-lg p-2 max-h-44 overflow-y-auto custom-scrollbar space-y-1">
                {loreEntries.length === 0 ? (
                  <p className="text-[10px] text-steel-600 italic text-center py-4">No lore entries yet</p>
                ) : (
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 p-2 rounded-lg text-left transition-all text-xs ${
                          linked ? 'bg-lore-500/10 border border-lore-500/30' : 'hover:bg-ink-900 border border-transparent'
                        }`}
                      >
                        <span className={config.color}>{config.icon}</span>
                        <span className="text-steel-200 flex-1 truncate">{entry.name}</span>
            ...
            <div className="flex items-start gap-6">
              <div className="w-44 flex-shrink-0">
                <div className="flex items-center justify-between mb-2">
                  <label className="text-[9px] font-bold text-steel-600 uppercase tracking-[0.2em]">Eras</label>
                  <button onClick={addEra} className="p-1 hover:bg-ink-700 rounded text-char-400"><Plus size={13} /></button>
                </div>
                    ...
                    <div
                      ...
                      className={`group flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer transition-all border text-xs ${
                        activeEraId === era.id
                          ...
                          : 'bg-ink-950 border-ink-700 text-steel-400 hover:border-steel-600'
                      }`}
                    >
                  ...
                  <div className="animate-fade-in space-y-5">
                    <div className="flex gap-4">
                      <div className="flex-1">
                        <label className="block text-[9px] font-bold text-steel-600 uppercase tracking-[0.2em] mb-1.5">Era Name</label>
                        <input
                          type="text" value={currentEra.name}
                          onChange={e => updateEra(currentEra.id, { name: e.target.value })}
                          className="w-full bg-ink-950 border border-ink-700 rounded-lg px-3.5 py-2 text-steel-100 text-sm"
                        />
                      </div>
                      <div className="w-28">
                        <label className="block text-[9px] font-bold text-steel-600 uppercase tracking-[0.2em] mb-1.5">Age App.</label>
                        <input
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        <label className="text-[9px] font-bold text-steel-600 uppercase tracking-[0.2em]">Visual Prompt Tags</label>
                      ...
                      <textarea
                        ...
                        className="w-full bg-ink-950 border border-ink-700 rounded-lg p-3 text-xs text-steel-200 font-mono leading-relaxed"
                      />
                    </div>

                    {/* Prompt preview */}
                    <div className="bg-ink-900 border border-ink-700 p-4 rounded-xl">
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-[9px] font-bold text-steel-600 uppercase tracking-[0.2em]">Master Prompt Preview</p>
                        <button onClick={handleCopy} className="flex items-center gap-1.5 px-3 py-1 bg-ink-800 border border-ink-700 rounded-lg text-[10px] font-bold text-steel-400 hover:text-white transition-all">
                          {copied ? <Check size={12} className="text-emerald-500" /> : <Copy size={12} />}
                          {copied ? 'Copied' : 'Copy'}
                        </button>
                      </div>
                      <p className="text-xs text-char-400 font-mono italic p-3 bg-ink-950 rounded border border-char-400/20 leading-relaxed">
                        {generateMasterPrompt(formData as Character, activeEraId)}
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="h-64 flex flex-col items-center justify-center text-steel-600 opacity-40">
        ...
        {/* Footer */}
        <div className="p-5 border-t border-ink-700 bg-ink-900/50 flex items-center justify-between flex-shrink-0">
          <button onClick={() => setCharacterEditorOpen(false)} className="px-5 py-2 text-steel-400 hover:text-white font-medium transition-colors text-sm">Cancel</button>
