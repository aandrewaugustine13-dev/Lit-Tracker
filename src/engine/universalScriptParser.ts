...
function countStructuredPanels(scriptText: string): { pages: number; panels: number } {
  ...
  const panelMatches = scriptText.match(/^\s*(?:[-*#>]+\s*)?PANEL\s+(?:#\s*)?(?:\d+|ONE|TWO|THREE|FOUR|FIVE|SIX|SEVEN|EIGHT|NINE|TEN|ELEVEN|TWELVE|THIRTEEN|FOURTEEN|FIFTEEN|SIXTEEN|SEVENTEEN|EIGHTEEN|NINETEEN|TWENTY)\b/gim) || [];
  return {
...
function buildDeterministicPanelStructure(scriptText: string): Array<{ page_number: number; panels: Array<{ panel_number: number; lines: string[] }> }> {
  ...
  for (const rawLine of lines) {
    ...
    const panelMatch = line.match(/^(?:[-*#>]+\s*)?PANEL\s+(?:#\s*)?(\d+|ONE|TWO|THREE|FOUR|FIVE|SIX|SEVEN|EIGHT|NINE|TEN|ELEVEN|TWELVE|THIRTEEN|FOURTEEN|FIFTEEN|SIXTEEN|SEVENTEEN|EIGHTEEN|NINETEEN|TWENTY)\b/i);
    if (panelMatch) {
      const parsedPanelNumber = parseNumberToken(panelMatch[1]);
      currentPanel = Number.isFinite(parsedPanelNumber) ? parsedPanelNumber : currentPanel + 1;
